...
logging.basicConfig(filename='my-316.log', level=logging.DEBUG, format=LOG_FORMAT)
...
db = pymysql.connect('rm-2zeb835794039706ebo.mysql.rds.aliyuncs.com',user = "camore",passwd = "camore",db = "medstore")
...
def updateSQL(sql):
   ...
   try:
      cursor.execute(sql)
      db.commit()
      # logging.info('insert语句受影响的行数：%s  %s',cursor.rowcount,cursor.lastrowid )
      return {
         'code':200,
         'count':cursor.rowcount,
         'lastId':cursor.lastrowid,
